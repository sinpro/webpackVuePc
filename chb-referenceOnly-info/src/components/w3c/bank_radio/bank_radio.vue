...
export default {
  ...
  methods: {
    ...
    handlerKeyDownEnter(e) {
      // console.log('e', e);
      try {
        let codestr = e.code || e.key;
        if (e.keyCode === 32 || ['Space'].includes(codestr)) {
          this.model = this.isDisabled ? this.model : this.label;
          this.clearDefault(e);
          this.spaceFun(); //兼容靓仔空格关闭的逻辑
        }
        let isEnter = !!(e.keyCode === 13 || ['Enter'].includes(codestr));
        if (this.isTheSpecialCase && isEnter) {
          this.model = this.isDisabled ? this.model : this.label;
          this.clearDefault(e);
        }
        return false;
