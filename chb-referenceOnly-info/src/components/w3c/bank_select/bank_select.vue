<template>
  <div
    ...
    :class="[
      ...
      selectDisabled || readonly ? 'barrier_free_text_wrap' : ''
    ]"
  ...
  >
    ...
    <!-- 处理disable和readonly 情况下,语音无法识别问题 ====== start ======  @keydown="(e)=>e.preventDefault()" -->
    <div
      v-if="isNeedFakeText && (selectDisabled || readonly)"
      class="barrier_free_text input__inner el-input el-input--suffix"
    >
      {{ theInputFakeText }}
    </div>
    <!-- 处理disable和readonly 情况下,语音无法识别问题 ====== start ====== -->
    ...
    >
      ...
      >
        ...
        >
          <bank_option :value="query" created v-if="showNewOption"> </bank_option>
          <slot></slot>
        </bank_scrollbar>
        <template
          v-if="
...
import ElOption from './bank_option.vue';
...
export default {
  ...
  computed: {
    ...
    theInputFakeText() {
      ...
      return this.selectedLabel || this.currentPlaceholder || '';
    }
  },
  ...
  mounted() {
    this.$nextTick(() => {
      const mainDiv = document.getElementsByClassName('cont-wrap')[0];
      mainDiv.addEventListener('scroll', this.blur);
    });
  ...
  beforeDestroy() {
    const mainDiv = document.getElementsByClassName('cont-wrap')[0];
    mainDiv.removeEventListener('scroll', this.blur);
