      ...
      >
        <div class="el-message-box__header" v-if="title !== null">
          ...
          <!-- <button 
            ...
            :title="store.getters.getLang == 'zh-CN'?'关闭':store.getters.getLang == 'zh-HK'?'關閉':'close'"
            v-if="false"
            @click="handleAction(distinguishCancelAndClose ? 'close' : 'cancel')"
            @keydown.enter="handleAction(distinguishCancelAndClose ? 'close' : 'cancel')">
            <i class="el-message-box__close el-icon-close"></i>
            <i class="the_visible_icon_for_read">{{store.getters.getLang == 'zh-CN'?'关闭':store.getters.getLang == 'zh-HK'?'關閉':'close'}}</i>
          </button> -->
          <button
            type="button"
            class="el-message-box__headerbtn"
          ...
          >
            <i class="el-message-box__close el-icon-close"></i>
            <!--方法1: 解决测试提的bug,elementui 弹框被识别为 close问题 -->
        ...
        <div class="el-message-box__content">
          ...
          <div class="el-message-box__message" v-if="message !== ''">
            <slot>
              <p id="my-msg" tabindex="0" v-if="!dangerouslyUseHTMLString">
                {{ message }}
              </p>
              <p id="my-msg" tabindex="0" v-else v-html="message"></p>
            </slot>
          </div>
...
export default {
  ...
  computed: {
    ...
    myAriaLabel() {
      return this.title || i18n.t('common.Dialog');
    }
  },
  ...
  methods: {
    ...
    handleAction(action) {
      ...
      try {
        setTimeout(() => {
          let nextDom = document.getElementById(this.nextFocusId);
          if (nextDom) {
            nextDom.setAttribute('tabindex', '0');
            nextDom.focus();
          } else {
            if (document.getElementById('breadCrumb')) {
              document.getElementById('breadCrumb').focus();
            }
          }
        }, 50);
      } catch (error) {
      ...
      if (val) {
        ...
        if (this.$type === 'alert' || this.$type === 'confirm') {
          this.$nextTick(() => {
            ...
            document.querySelector('#my-msg').focus();
          });
        }
